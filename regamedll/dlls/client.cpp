...
#include "amxx/runtime.h"
#include "amxx/admin.h"
#include "amxx/native_entities.h"
...
BOOL EXT_FUNC ClientConnect(edict_t *pEntity, const char *pszName, const char *pszAddress, char *szRejectReason)
{
	...
	params[0].val = ENTINDEX(pEntity);
	params[0].size = 0;
	AMXXRuntime::GetInstance().ExecuteForwardEx("client_connect", 1, params);
...
void EXT_FUNC ClientDisconnect(edict_t *pEntity)
{
	...
	params[0].val = ENTINDEX(pEntity);
	params[0].size = 0;
	AMXXRuntime::GetInstance().ExecuteForwardEx("client_disconnect", 1, params);

    // AMXX 1.8.3: client_disconnected / client_remove
    AMXXRuntime::GetInstance().ExecuteForwardEx("client_disconnected", 1, params);
    AMXXRuntime::GetInstance().ExecuteForwardEx("client_remove", 1, params);
...
void EXT_FUNC ClientKill(edict_t *pEntity)
{
	...
	// Trigger client_kill forward (id) before killing
	ForwardCallParam killParams[1];
	killParams[0].type = FP_CELL;
	killParams[0].val = ENTINDEX(pEntity);
	killParams[0].size = 0;
	int killResult = AMXXRuntime::GetInstance().ExecuteForwardEx("client_kill", 1, killParams);
	if (killResult == PLUGIN_HANDLED || killResult == PLUGIN_STOP)
		return;
...
void EXT_FUNC ClientPutInServer(edict_t *pEntity)
{
	...
	params[0].val = ENTINDEX(pEntity);
	params[0].size = 0;
	AMXXRuntime::GetInstance().ExecuteForwardEx("client_putinserver", 1, params);

	// AMXX: client_authorized(id, authid[]) fires once the player is fully
	// authenticated and placed in the server. For Steam this is after ticket
	// validation; for LAN/listen server it fires immediately on putinserver.
	{
		const char *authid = GETPLAYERAUTHID(pEntity);
		if (!authid) authid = "";
		ForwardCallParam authParams[2];
		authParams[0].type = FP_CELL;
		authParams[0].val = ENTINDEX(pEntity);
		authParams[0].size = 0;
		authParams[1].type = FP_STRING;
		authParams[1].str = authid;
		authParams[1].size = 0;
		AMXXRuntime::GetInstance().ExecuteForwardEx("client_authorized", 2, authParams);
	}
...
void EXT_FUNC ClientCommand_(edict_t *pEntity)
{
	...
	if (AMXXAdminSystem::GetInstance().ExecuteCommand(command, pEntity)) {
		return;
	}
...
void EXT_FUNC ServerDeactivate()
{
	...
	// 通知 AMXX 子系统：地图即将结束（执行 plugin_end、断开玩家）
	AMXXRuntime::GetInstance().OnServerDeactivate();
	...
	// 清理 AMXX 所有跨地图残留状态（tasks/forwards/events/hooks/globals）
	AMXXRuntime::GetInstance().OnServerDeactivatePost();
}
...
void EXT_FUNC ServerActivate(edict_t *pEdictList, int edictCount, int clientMax)
{
	...
	AMXXRuntime::GetInstance().OnServerActivate();
}
...
{
	// AMXX FM_AddToFullPack forward
	if (FireFMForwardAddToFullPack((entity_state_t *)state, e, ENTINDEX(ent), ENTINDEX(host), hostflags, player, pSet))
		return FALSE;
...
void EXT_FUNC UpdateClientData(const edict_t *ent, int sendweapons, struct clientdata_s *cd)
{
	...
	// AMXX FM_UpdateClientData forward
	if (FireFMForwardUpdateClientData(ENTINDEX(ent), sendweapons, (clientdata_t *)cd))
		return;
...
void EXT_FUNC CmdStart(const edict_t *pEdict, const struct usercmd_s *cmd, unsigned int random_seed)
{
	...
	// AMXX FM_CmdStart forward
	if (FireFMForwardCmdStart(ENTINDEX(pEdict), (usercmd_t *)cmd, random_seed))
		return;
