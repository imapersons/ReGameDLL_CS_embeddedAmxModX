...
void DispatchTouch(edict_t *pentTouched, edict_t *pentOther)
{
	...
	{
		// P1-10: Ham_Touch 桥接 (无 ReGame hookchain, 在 dispatch 层触发)
		if (AMXXHamSandwich::GetInstance().HasCallbacks(Ham_Touch))
		{
			int touchedIdx = ENTINDEX(pentTouched);
			int otherIdx = ENTINDEX(pentOther);
			cell result = AMXXHamSandwich::GetInstance().DispatchHam(Ham_Touch, 2,
				(cell)touchedIdx, (cell)otherIdx);
...
void DispatchUse(edict_t *pentUsed, edict_t *pentOther)
{
	...
	{
		// P1-10: Ham_Use 桥接 (无 ReGame hookchain, 在 dispatch 层触发)
		// 原版签名: Ham_Use(this, caller, activator, useType, Float:value)
		if (AMXXHamSandwich::GetInstance().HasCallbacks(Ham_Use))
		{
			int usedIdx = ENTINDEX(pentUsed);
			int otherIdx = pentOther ? ENTINDEX(pentOther) : 0;
			float useValue = 0.0f;
			cell result = AMXXHamSandwich::GetInstance().DispatchHam(Ham_Use, 5,
				(cell)usedIdx, (cell)otherIdx, (cell)otherIdx, (cell)USE_TOGGLE, amx_ftoc(useValue));
			if (result == HAM_SUPERCEDE || result == HAM_OVERRIDE)
				return;
		}
