...
int AMXAPI amx_Callback(AMX *amx, cell index, cell *result, cell *params)
{
  ...
  amx_GetNative(amx, index, funcname);
#if defined AMXX_VERBOSE_TRACE
  AMXX_LOG("Calling native: %s (index=%d, params=%d bytes)", funcname, index, params[0]);
#endif
...
#if defined AMXX_VERBOSE_TRACE
  AMXX_LOG("Native %s returned: %d", funcname, *result);
#endif
...
int AMXAPI amx_Init(AMX *amx, void *program)
{
  ...
  {
    AMXX_LOG("Already initialized (AMX_FLAG_RELOC set)");
    return AMX_ERR_INIT;
  }
  ...
  if (hdr->magic!=AMX_MAGIC)
  {
    AMXX_LOG("Invalid magic: 0x%04X (expected 0x%04X)", hdr->magic, AMX_MAGIC);
    return AMX_ERR_FORMAT;
  }
  if (hdr->file_version<MIN_FILE_VERSION || hdr->amx_version>CUR_FILE_VERSION)
  {
    AMXX_LOG("Version mismatch: file_version=%d, amx_version=%d (min=%d, cur=%d)",
             hdr->file_version, hdr->amx_version, MIN_FILE_VERSION, CUR_FILE_VERSION);
    return AMX_ERR_VERSION;
  }
  if (hdr->defsize!=sizeof(AMX_FUNCSTUB) && hdr->defsize!=sizeof(AMX_FUNCSTUBNT))
  {
    AMXX_LOG("Invalid defsize: %d (expected %d or %d)",
             hdr->defsize, sizeof(AMX_FUNCSTUB), sizeof(AMX_FUNCSTUBNT));
    return AMX_ERR_FORMAT;
  }
  if (USENAMETABLE(hdr)) {
    uint16_t *namelength;
    ...
    {
      AMXX_LOG("Name length exceeds max: %d (max=%d)", *namelength, sNAMEMAX);
      return AMX_ERR_FORMAT;
    }
  }
  if (hdr->stp<=0)
  {
    AMXX_LOG("Invalid stp: %d", hdr->stp);
    return AMX_ERR_FORMAT;
  }

  AMXX_LOG_DBG("AMX Header Info: size=%d, cod=%d, dat=%d, hea=%d, stp=%d, cip=%d",
           hdr->size, hdr->cod, hdr->dat, hdr->hea, hdr->stp, hdr->cip);
  AMXX_LOG_DBG("Publics: %d, Natives: %d, Libraries: %d, PubVars: %d, Tags: %d",
           NUMENTRIES(hdr, publics, natives),
           NUMENTRIES(hdr, natives, libraries),
           NUMENTRIES(hdr, libraries, pubvars),
           NUMENTRIES(hdr, pubvars, tags),
           NUMENTRIES(hdr, tags, cod));

  assert((hdr->flags & AMX_FLAG_COMPACT)!=0 || hdr->hea == hdr->size);
  if ((hdr->flags & AMX_FLAG_COMPACT)!=0) {
    #if AMX_COMPACTMARGIN > 2
      AMXX_LOG_DBG("Expanding compact code");
      expand((unsigned char *)program+(int)hdr->cod,
             hdr->size - hdr->cod, hdr->hea - hdr->cod);
    #else
      AMXX_LOG("AMX_COMPACTMARGIN too small for expand");
      return AMX_ERR_FORMAT;
    #endif
  }
  ...
  * (cell *)(amx->base+(int)hdr->stp-sizeof(cell)) = 0;
  ...
  AMXX_LOG_DBG("Initial values: hlw=%d, stp=%d, hea=%d, stk=%d",
           amx->hlw, amx->stp, amx->hea, amx->stk);

  hook = (BROWSEHOOK)amx->usertags[UT_BROWSEHOOK];
  if (hook)
  {
    AMXX_LOG_DBG("Calling browse hook");
    hook(amx, NULL, NULL);
  }

  AMXX_LOG_DBG("Relocating code...");
  result = amx_BrowseRelocate(amx);
  if (result != AMX_ERR_NONE)
  {
    AMXX_LOG("amx_BrowseRelocate failed: %d", result);
    return result;
  }

  AMXX_LOG_FUNC_RET(AMX_ERR_NONE);
  return AMX_ERR_NONE;
}
  ...
  for (i=0; i<numnatives; i++) {
    if (func->address==0) {
      ...
      if (funcptr!=NULL)
      {
        ...
        no_function = GETENTRYNAME(hdr,func);
        AMXX_LOG("[AMXX] Native not found during registration: '%s'", no_function);
...
int AMXAPI amx_Exec(AMX *amx, cell *retval, int index)
{
  ...
  if (amx->callback==NULL)
  {
    AMXX_LOG("Callback is NULL");
    return AMX_ERR_CALLBACK;
  }
  if (!(amx->flags & AMX_FLAG_PRENIT))
	if ((amx->flags & AMX_FLAG_NTVREG)==0)
	{
      AMXX_LOG("Natives not registered");
      return AMX_ERR_NOTFOUND;
	}
  if ((amx->flags & AMX_FLAG_RELOC)==0)
  {
    AMXX_LOG("Code not relocated");
    return AMX_ERR_INIT;
  }
  ...
  if (index==AMX_EXEC_MAIN) {
    if (hdr->cip<0)
    {
      AMXX_LOG("Invalid main entry point: cip=%d", hdr->cip);
      return AMX_ERR_INDEX;
    }
    AMXX_LOG_DBG("Executing main entry point (cip=%d)", hdr->cip);
    cip=(cell *)(code + (int)hdr->cip);
  } else if (index==AMX_EXEC_CONT) {
    AMXX_LOG_DBG("Continuing execution from: cip=%d", amx->cip);
    frm=amx->frm;
    stk=amx->stk;
    hea=amx->hea;
    pri=amx->pri;
    ...
    AMXX_LOG("Invalid negative index: %d", index);
    return AMX_ERR_INDEX;
  } else {
    if (index>=(cell)NUMENTRIES(hdr,publics,natives))
    {
      AMXX_LOG("Invalid public index: %d (max=%d)", index, NUMENTRIES(hdr, publics, natives)-1);
      return AMX_ERR_INDEX;
    }
    func=GETENTRY(hdr,publics,index);
    amx_GetPublic(amx, index, funcname);
    AMXX_LOG_DBG("Executing public: %s (index=%d, address=%d)", funcname, index, func->address);
  ...
  AMXX_LOG_DBG("Execution finished, result=%d, retval=%d", i, retval ? *retval : 0);
  AMXX_LOG_FUNC_RET(i);
  return i;
  ...
  for ( ;; ) {
    ...
    case OP_HALT:
      GETPARAM(offs);
      AMXX_LOG_DBG("OP_HALT: exit_code=%d, retval=%d", offs, pri);
      if (retval!=NULL)
        *retval=pri;
      amx->frm=frm;
      amx->pri=pri;
...
#endif /* AMX_XXXSTRING || defined AMX_EXEC */
